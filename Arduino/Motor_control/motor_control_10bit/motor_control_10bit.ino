...
#define ONLY_NEUTRAL  7
#define TURN_PIN      8
#define FWRD_BKWRD    2
#define OUT_TURN      4
#define OUT_ONLY_NEU  5

void setup(){
  Serial.begin(9600);
  pinMode (MOTOR_1_PIN, OUTPUT);
  pinMode (ONLY_NEUTRAL, INPUT_PULLUP);
  pinMode (TURN_PIN, INPUT_PULLUP);
  pinMode (FWRD_BKWRD, OUTPUT);
  pinMode (OUT_TURN, OUTPUT);
  pinMode (OUT_ONLY_NEU, OUTPUT);
  digitalWrite (FWRD_BKWRD, LOW);
  digitalWrite (OUT_TURN, LOW);
  digitalWrite (OUT_ONLY_NEU, LOW);
...
//  Serial.println(val);
  val = 0;
  analogWrite(MOTOR_1_PIN, val);
  int turn_pin_val = digitalRead(TURN_PIN);
  Serial.println(turn_pin_val);
  if (!digitalRead(ONLY_NEUTRAL))
  {
    digitalWrite(OUT_TURN, turn_pin_val);
  }
  else
  {
    digitalWrite(OUT_TURN, LOW);
